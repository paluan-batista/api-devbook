...
// NovoRepositoryDePublicacoes cria um repositório de publicações
func NovoRepositoryDePublicacoes(db *sql.DB) *Publicacoes {
...
func (repository Publicacoes) Buscar(usuarioID uint64) ([]models.Publicacao, error) {
...
func (repository Publicacoes) Curtir(publicacaoID uint64) error {
	statement, erro := repository.db.Prepare("update publicacoes set curtidas = curtidas + 1 where id = ?")
	if erro != nil {
		return erro
	}
	defer statement.Close()
